...
python3 bin/trainer.py \
    ...
    --max-duration 10 --model-name valle \
    --exp-dir exp/valle
    --dtype "bfloat16" \
"""
import pathlib
...
def get_parser():
    ...
    parser.add_argument(
        "--train_dir",
        default='MyTTSDataset/train_tune'
    )
...
def train_one_epoch(
    ...
    while True:
        ...
        try:
            ...
            ############wantb新增loss##############
            avg_loss = loss_info["loss"] / max(loss_info["frames"], 1)
            lb_loss  = loss_info.get("LoadBalanceLoss", 0.0) / max(loss_info["frames"], 1)
            ############wantb新增loss##############
            ...
            total_train_loss_avg += loss_info["loss"]
            total_train_frames_avg += loss_info["frames"]
            ...
            ################printfreez#########
            if batch_idx == 1:
                print("==== Trainable parameters with grad in first batch ====")
        ...
        if batch_idx % params.log_interval == 0:
            ...
            if rank == 0:
                wandb.log({
                    ...
                    "train/lr":        cur_lr,
                    "train/loss":      avg_loss,
                    "train/LoadBalanceLoss": lb_loss,
                    "train/epoch":     params.cur_epoch,
                    "train/step":      params.batch_idx_train
                })
            ######################wantbedit##############
        ...
        if params.batch_idx_train % params.valid_interval == 0:
            ...
            if rank == 0:
                wandb.log({
                    "epoch":                  params.cur_epoch,
                    "valid/loss": (
                        valid_info["loss"] / max(valid_info["frames"], 1)
                    ),
                    "valid/LoadBalanceLoss": (
                        valid_info.get("LoadBalanceLoss", 0.0)
                        / max(valid_info["frames"], 1)
                    ),
                    "valid/epoch": params.cur_epoch,
                    "valid/step":  params.batch_idx_train,
                })

            ##############wantb####################

    if total_train_frames_avg > 0:
        train_loss_avg = total_train_loss_avg / total_train_frames_avg
        if rank == 0:
            wandb.log({
                "epoch": params.cur_epoch,
...
def run(rank, world_size, args):
    ...
    if rank == 0:
        wandb.init(
            project="valle-training",           # 你的專案名稱
            name=f"{args.exp_dir.name}",        # 這次 run 的名稱
            config=vars(args)                   # 所有 CLI 參數
        )
    ...
    # def freeze_except_last_n(layer_list, n_last: int = 2):
    #     """將 layer_list 前面部分的參數全部凍結，只開啟最後 n_last 層。"""
    #     n_total = len(layer_list)
    #     for idx, layer in enumerate(layer_list):
    #         trainable = idx >= n_total - n_last
    #         for p in layer.parameters():
    #             p.requires_grad = trainable
    
    # # ---------- AR decoder ----------
    # freeze_except_last_n(model.ar_decoder.layers, n_last=2)

    # # ---------- NAR decoder ----------
    # # 1) 若實作是單一 NAR decoder (TransformerEncoder)
    # if hasattr(model, "nar_decoder"):
    #     freeze_except_last_n(model.nar_decoder.layers, n_last=2)

    # # 2) 若實作是多個 NAR decoder  (例如 list / ModuleList)
    # elif hasattr(model, "nar_decoders"):
    #     for nar_dec in model.nar_decoders:
    #         freeze_except_last_n(nar_dec.layers, n_last=2)

    # else:
    #     raise RuntimeError("找不到 nar_decoder / nar_decoders，請確認模型結構")
