    ...
    def forward(
        ...
        **kwargs,
        ...
        if train_stage in [0, 1]:
            ...
            xy_dec,moe_loss = self.ar_decoder(
                (xy_pos, None),
                mask=xy_attn_mask,
            )
            if isinstance(xy_dec, tuple):
                xy_dec = xy_dec[0]
            ...
            ##edit,because of transformer decoder return not only output but also moe total loss add  orginal: logits = self.ar_predict_layer(xy_dec[:, x_len:]).permute(0, 2, 1)
            logits = self.ar_predict_layer(xy_dec[:, x_len:]).permute(0, 2, 1)
            
            # loss            
            total_loss = F.cross_entropy(logits, targets, reduction=reduction)

            # 取得 load balance loss（如果有）

            moe_loss = getattr(self.ar_decoder, "total_moe_loss", 0.0)
            moe_loss_weight = 0.001
            if not isinstance(moe_loss, torch.Tensor):
                moe_loss = torch.tensor(moe_loss, device=x.device, dtype=torch.float32)

            total_loss += moe_loss_weight * moe_loss
            metrics["LoadBalanceLoss"] = moe_loss * x.shape[0]  
        ...
        if train_stage in [0, 2]:
            ...
            xy_dec, moe_loss = self.nar_decoder(
            ...
            ############原始######### 
             ...
             ##edit,because of transformer decoder return not only output but also moe total loss add  orginal: xy_dec = xy_dec[:, x_lens.max() + prefix_len :]
    ...
    def inference(
        ...
        if self.prefix_mode == 0:
            ...
            for i, (predict_layer, embedding_layer) in enumerate(
                ...
                if isinstance(xy_dec, tuple):   # ← 新增
                    xy_dec = xy_dec[0]          # 只取真正的 decoder 輸出 Tensor
                #############edit#############
